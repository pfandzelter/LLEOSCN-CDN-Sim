# Authors: Ben S. Kempton, Tobias Pfandzelter
...
class Constellation():
    ...
    def __init__(
        ...
        self.gnd_links_array = np.zeros(NUM_GROUND_POINTS, dtype=LINK_DTYPE)
    ...
    def setConstillationTime(self, time=0.0):
        ...
        #print("updating to", self.current_time)

        #print("Updating satellite positions...")

        # update all the satellite positions
        # rotate the pos to time = 0 as we consider the earth's positions then
        ...
        neg_rotation_matrix = self.getRotationMatrix(EARTH_ROTATION_AXIS,
                                                 -degrees_to_rotate)

        for sat_id in range(self.satellites_array.size):
            #print("updating", sat_id)

            plane = self.satellites_array[sat_id]["plane_number"]

            time = self.current_time + self.satellites_array[sat_id]["time_offset"]

            pos = self.plane_solvers[plane].xyzPos(time)
        ...
        #print("Updating ground positions...")
        for gnd_pt in range(self.groundpoints_array.size):
            #print("updating gnd", gnd_pt)
            initial_pos = self.groundpoints_array[gnd_pt][
                ["init_x", "init_y", "init_z"]]
            initial_pos = [initial_pos[0], initial_pos[1], initial_pos[2]]

            pos = np.dot(rotation_matrix, initial_pos)

            self.groundpoints_array[gnd_pt]["x"] = np.int32(pos[0])
            self.groundpoints_array[gnd_pt]["y"] = np.int32(pos[1])
            self.groundpoints_array[gnd_pt]["z"] = np.int32(pos[2])
    ...
    def generateNetworkSubgraph(self, city_names, source, target):
        """ Makes a igraph graph of the network at the current time but does not consider unused cities.
        ...
        for lnk_idx in range(self.total_links):
            if self.link_array[lnk_idx]["node_1"] < 0 and not str(self.link_array[lnk_idx]["node_1"]) == source and not str(self.link_array[lnk_idx]["node_1"]) == target:
                #print("not adding", self.link_array[lnk_idx]["node_1"], "not either or", source, target)
                continue
            
            if self.link_array[lnk_idx]["node_2"] < 0 and not str(self.link_array[lnk_idx]["node_2"]) == source and not str(self.link_array[lnk_idx]["node_2"]) == target:
                #print("not adding", self.link_array[lnk_idx]["node_2"], "not either or", source, target)
                continue
    ...
    def generateSatelliteShortestPaths(self, sats_to_consider=None):
        """ Calculate the shortest paths between given satellites at the given time.
        ...
        if sats_to_consider is None:
            sats_to_consider = range(num_sat)

        shortest_paths = [[[] for x in range(num_sat)] for x in range(num_sat)] 
        for sat in tqdm(sats_to_consider, desc="Generating Satellite Shortest Paths"):
            consider_range = []
            for x in sats_to_consider:
                if x >= sat:
                    consider_range.append(x)

            consider_sat = G.vs.select(consider_range)
            
            sp = G.get_shortest_paths(v=sat, to=consider_sat, weights="weight")

            weighted_paths = [[] for x in range(num_sat)]
            ...
            for path in sp:
                ...
                pathweight = 0
                for eid in eids:
                    pathweight += G.es[eid]["weight"]

                weighted_paths[path[-1]] = [path, pathweight]
            
            shortest_paths[sat] = weighted_paths

        return shortest_paths


    # only consider "best" satellite
    def getGndToSatLinks(self, nodes_to_consider):
        links = dict((n["node_1"], [n["node_2"], n["distance"]]) for n in (l for l in tqdm(self.gnd_links_array, desc="Getting gnd_sat_links") if l["node_1"] != 0))
    ...
    def calculatePlusGridLinks(
        ...
        self.gnd_links_array = np.zeros(len(self.groundpoints_array), dtype=LINK_DTYPE)
                ...
                for sat_idx in range(self.total_sats):
                    ...
                    if d < max_stg_range:
                        if link_idx < self.link_array_size - 1:
                            ...
                            if d < shortest:
                                self.gnd_links_array[gnd_link_id]["node_1"] = gnd_id
                                self.gnd_links_array[gnd_link_id]["node_2"] = sat_id
                                self.gnd_links_array[gnd_link_id]["distance"] = np.int32(d)
                                shortest = d
                            link_idx = link_idx + 1
                        else:
            ...
            for sat_idx in range(total_sats):
                ...
                if d < max_stg_range:
                    if link_idx < link_array_size - 1:
                        ...
                        if d < shortest:
                            gnd_links_array[gnd_link_id]["node_1"] = gnd_id
                            gnd_links_array[gnd_link_id]["node_2"] = sat_id
                            gnd_links_array[gnd_link_id]["distance"] = np.int32(d)
                            shortest = d
                        link_idx = link_idx + 1
                    else:
                        print("ERROR! ran out of room in the link array")
